...
The `Technical Reference` section is the authoritative command list for the current CLI.
...
Current CLI report scope:

- `report` prints the current all-time report for the analyzed local dataset.
- Daily and weekly report windows are part of the active plan and are not implemented yet.
...
- workflow clustering and all-time CLI reporting
...
- daily and weekly report windows
...
- `report` currently operates on the full analyzed dataset.
- Date-scoped daily and weekly reports are planned but not yet available.
...
- `report`는 분석된 로컬 데이터 전체를 기준으로 한 누적 리포트를 출력합니다.
- daily, weekly 기간 리포트는 active plan에 포함되어 있지만 아직 구현되지 않았습니다.
...
- daily 및 weekly 기간 리포트
- 단기 구간용 emerging workflow 요약
...
- `report`는 지금은 전체 누적 데이터 기준으로만 동작합니다.
- 날짜 기준 daily/weekly 리포트는 아직 CLI에 추가되지 않았습니다.
...
- `report` 目前只输出基于已分析本地数据的 all-time 报告。
- daily 和 weekly 时间窗口已经进入计划，但尚未实现。
...
- 第一个用于 macOS 前台应用和聚焦窗口变化的原生 Swift 采集脚本
- 工作流聚类与当前 all-time CLI 报告输出
- 对工作流进行重命名、排除、重新包含、隐藏、取消隐藏的反馈持久化
- 会话列表与会话删除后自动重新分析
- 可安全发送给 LLM 的摘要工作流 payload 导出
- 基于 OpenAI Responses API 的摘要工作流分析适配器
- 使用 macOS Keychain 存储 OpenAI API key

### 尚未实现的功能

- daily 与 weekly 报告窗口
- 面向短时间范围的 emerging workflow 摘要
- Windows 点击、文件操作、剪贴板采集器
- 桌面 UI
- 工作流反馈 UI
- OpenAI 之外的其他 LLM provider 集成
- 非 macOS 平台上的安全凭证存储

### 分析流程

1. 从 mock 数据、导入文件、本地 ingest 服务或 Windows / macOS active-window 采集器接收原始事件。
...
- 如需运行 macOS active-window 采集器，需要 Xcode 或 Xcode Command Line Tools 中的 Swift
- 如果未将 key 存入安全存储，运行 `llm:analyze` 需要 `OPENAI_API_KEY`
...
- `report` 目前基于完整已分析数据集生成报告。
- 按日期的 daily 与 weekly 报告仍在计划中。
...
- `report` は、分析済みローカルデータ全体を対象にした all-time レポートを出力します。
- daily / weekly の期間別レポートは計画中ですが、まだ未実装です。
...
- macOS の前面アプリとフォーカスウィンドウ変化を収集する最初のネイティブ Swift コレクタ
- ワークフロークラスタリングと現在の all-time CLI レポート出力
- 名前変更、除外、再表示、非表示解除を含むワークフローフィードバック保存
- セッション一覧表示と削除後の自動再分析
- LLM に送っても比較的安全な要約ワークフロー payload 出力
- OpenAI Responses API を使った要約ワークフロー分析アダプタ
- macOS Keychain を使った OpenAI API キーのセキュア保存

### まだ未実装の機能

- daily / weekly のレポートウィンドウ
- 短期間向けの emerging workflow 要約
- クリック、ファイル操作、クリップボード向け Windows コレクタ
- デスクトップ UI
- ワークフローフィードバック UI
- OpenAI 以外の追加 LLM provider 連携
- macOS 以外のプラットフォームでのセキュア認証情報保存

### 分析パイプライン

1. mock データ、import ファイル、ローカル ingest サーバー、Windows / macOS の active-window コレクタから Raw Event を取得します。
...
- macOS active-window コレクタを動かす場合は Xcode または Xcode Command Line Tools の Swift
- セキュア保存にキーを入れていない場合、`llm:analyze` には `OPENAI_API_KEY` が必要
...
- `report` は現在、分析済みデータ全体に対して動作します。
- 日次・週次の期間別レポートはまだ CLI に入っていません。
