...
function parseRunnerOptions(argv: string[]): RunnerOptions {
  const options: RunnerOptions = {
    ingestUrl: "",
    pollIntervalMs: DEFAULT_GWS_SHEETS_POLL_INTERVAL_MS,
    lookbackMs: DEFAULT_GWS_SHEETS_LOOKBACK_MS,
  };
...
async function postRawEvent(
  ingestUrl: string,
  event: ReturnType<typeof createSheetsContextRawEvent>,
  ...
  if (!response.ok) {
    throw new Error(`Ingest rejected Sheets signal with status ${String(response.status)}`);
  }
}
  ...
  while (!stopped) {
    try {
      ...
      for (const file of files) {
        ...
        const summary = getSpreadsheetSummary(file.id);
        const fingerprint = buildSpreadsheetFingerprint(file, summary);
